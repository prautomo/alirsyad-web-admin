@props(['name', 'type', 'step', 'label', 'value' => '', 'helper', 'data' => null, 'required'])

@if(@$type!=='hidden')
<div class="col-md-12">
    <div class="form-group">
        <label class="form-control-label" for="input-{{$name}}">{{$label}} {{@$required ? "(*)" : ""}}</label>
        <input id="{{$name}}" type="{{@$type?$type:'text'}}" {!! @!empty($step) ? 'step="'.$step.'"' : "" !!} name="{{$name}}" placeholder="{{$label}}" value="{{old($name) ?? $data[$name] ?? $value ?? ''}}" {{ $attributes->merge(['class' => " form-control ". ($errors->has($name) ? ' is-invalid' : '')]) }}>
